...
## สถานะปัจจุบัน: เฟส 0 — Bootstrap

โครงสร้างโปรเจค, Prisma schema เต็ม, auth (NextAuth v5 + dev-login shortcut) และ route/layout เปล่าๆ พร้อมแล้ว
ยังไม่มี business logic ของฟีเจอร์จริง (เช็คชื่อ, คำร้อง, แดชบอร์ด ฯลฯ) — มาในเฟสถัดไป
...
app/
  ...
  (admin)/admin/     เส้นทางฝั่งแอดมิน
  api/               Route handlers (auth, dev-login, cron, qr-token, checkin — ทยอยเพิ่มตามเฟส)
  login/, setup-profile/
lib/
...
prisma/
  ...
  seed.ts            ข้อมูลตัวอย่าง (คณะ/สาขา placeholder + dev users)
scripts/
...
2. เช็คชื่อสามประสาน (QR หมุนรหัส + GPS + กล้องหน้า + auto_approve/flag logic) — พร้อม tests

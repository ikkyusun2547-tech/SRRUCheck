...
(เฟส 6 — เริ่มแล้วบางส่วน: auth + dashboard)

## สถานะปัจจุบัน: เฟส 6 (บางส่วน) — แอปมือถือ: auth + dashboard + เช็คชื่อ
...
เฟส 6 เริ่มแล้วบางส่วน: แอปมือถือเข้าสู่ระบบได้ (dev-login shortcut), แสดงแดชบอร์ด Activity Passport
จริง, และเช็คชื่อได้ครบ (สแกน QR/แนบหลักฐาน + GPS + เซลฟี) ผ่าน Bearer-token auth bridge ที่เพิ่มใหม่
(ดู [`mobile/README.md`](mobile/README.md) สำหรับสถาปัตยกรรมและสิ่งที่ยังไม่ทำ — คำร้อง/ประวัติ/
แจ้งเตือน/โปรไฟล์/Google Sign-In จริง)
...
- **ข้อมูลสาธิต (ACT-DEMO-001/002/003) มี `startTime`/`endTime` แบบ relative-to-seed-time** — ถ้าเปิด
  session ใหม่ห่างจากตอน seed หลายชั่วโมง/ข้ามวัน กิจกรรมจะปิดรับเช็คชื่อเองไปแล้ว (ตามเวลาจริง ไม่ใช่
  บั๊ก) แก้ให้ `prisma/seed.ts`'s upsert เปลี่ยนจาก `update: {}` เป็น refresh
  `status`/`startTime`/`endTime` ทุกครั้งที่รัน — รัน `npm run prisma:seed` ซ้ำได้ตลอดเพื่อรีเซ็ตข้อมูล
  สาธิตกลับมาใช้งานได้ โดยไม่กระทบข้อมูลจริงอื่นๆ (upsert ยังทำงานแบบ idempotent ปกติ)
...
6. **แอปมือถือ (React Native/Expo)** 🚧 — เริ่มแล้วบางส่วน: Bearer-token auth bridge ฝั่ง backend
   (dev-login + Google JWKS verify, ใช้ user-upsert/domain-check ตัวเดียวกับเว็บ), `getSession()`
   รองรับทั้ง cookie (เว็บ) และ Bearer token (มือถือ) ใน 14 API routes ฝั่งนักศึกษา, CORS สำหรับ
   caller ข้าม origin, แอป Expo (expo-router) มีหน้าเข้าสู่ระบบ (dev-login list), แดชบอร์ด Activity
   Passport ดึงข้อมูลจริง, และเช็คชื่อครบวงจร (สแกน QR ด้วยกล้องหลัง/แนบหลักฐานด้วยตนเอง + ยืนยัน GPS
   เมื่อกิจกรรมต้องการ + ถ่ายเซลฟีด้วยกล้องหน้า ไม่มีทางเลือกอัปโหลดจากคลังภาพ) เรียก `/api/checkin`
   เส้นทางเดียวกับเว็บ — ยืนยันด้วย E2E ผ่าน Expo web build จริง (auth+dashboard 11/11, เช็คชื่อแบบ
   self-report ครบวงจรรวมถ่ายรูปจริงผ่าน fake camera stream 8/8) ยังไม่ทำ: คำร้อง 3 ประเภท, ประวัติ,
   แจ้งเตือน, โปรไฟล์, Google Sign-In จริงบนมือถือ (ดู [`mobile/README.md`](mobile/README.md))

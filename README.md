...
## สถานะปัจจุบัน: เฟส 6 (บางส่วน) — แอปมือถือ: auth + dashboard

เฟส 0-5 เสร็จสมบูรณ์ (ดูรายละเอียดในหัวข้อ "แผนการพัฒนา" ด้านล่าง) เฟส 6 เริ่มแล้วบางส่วน: แอปมือถือ
เข้าสู่ระบบได้ (dev-login shortcut) และแสดงแดชบอร์ด Activity Passport จริง โดยเรียก API เดียวกับเว็บ
ผ่าน Bearer-token auth bridge ที่เพิ่มใหม่ (ดู [`mobile/README.md`](mobile/README.md) สำหรับ
สถาปัตยกรรมและสิ่งที่ยังไม่ทำ — เช็คชื่อ/คำร้อง/ประวัติ/แจ้งเตือน/โปรไฟล์/Google Sign-In จริง)

เฟส 0-5 เสร็จแล้ว ครอบคลุมทั้งฝั่งนักศึกษา (auth, profile, เช็คชื่อสามประสาน, Activity Passport,
...
6. **แอปมือถือ (React Native/Expo)** 🚧 — เริ่มแล้วบางส่วน: Bearer-token auth bridge ฝั่ง backend
   ...
   caller ข้าม origin, แอป Expo (expo-router) มีหน้าเข้าสู่ระบบ (dev-login list) และแดชบอร์ด Activity
   Passport ดึงข้อมูลจริง — ยืนยันด้วย E2E ผ่าน Expo web build (11/11 scenario) ยังไม่ทำ: เช็คชื่อ
   (กล้อง/GPS/QR), คำร้อง 3 ประเภท, ประวัติ, แจ้งเตือน, โปรไฟล์, Google Sign-In จริงบนมือถือ (ดู

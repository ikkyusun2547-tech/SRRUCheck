...
## สถานะปัจจุบัน: เฟส 3 — แดชบอร์ดนักศึกษา + คำร้องต่างๆ

เฟส 0-3 เสร็จแล้ว: bootstrap, auth + profile setup, เช็คชื่อสามประสาน (QR หมุนรหัส + GPS + เซลฟี
กล้องหน้า + auto_approve/flag), และ Activity Passport + คำร้อง 3 ประเภท (กิจกรรมภายนอก, เทียบชั่วโมง
ผู้นำนักศึกษา, เช็คชื่อย้อนหลัง) ทดสอบ end-to-end กับ Supabase จริงผ่านหมดทุกสถานการณ์ รวมถึงยืนยัน
ตัวเลขบนแดชบอร์ดตรงกับที่คำนวณด้วยมือ 100% ยังไม่มี UI ฝั่งแอดมิน (อนุมัติคำร้อง, bulk/force
approve, ตารางเรียลไทม์, export, ตั้งค่าเกณฑ์/เพดานชั่วโมง) — มาในเฟส 4
...
app/
  ...
  api/               Route handlers (auth, dev-login, cron, qr-token, checkin, activities, requests, attendance)
  login/, setup-profile/
lib/
